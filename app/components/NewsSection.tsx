...
export default function NewsSection() {
  return (
    <section className="w-full bg-white py-16 pb-50">
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-20">
          ...
          <p className="mt-4 md:mt-0 max-w-[24%] text-justify text-gray-600 text-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {newsItems.map((item, idx) => (
            <Link href={item.link} key={idx} className="group cursor-pointer">
              <div className="relative w-full  h-140 overflow-hidden rounded-md bg-gray-200">
                <Image
                  src={item.image}
                  alt={item.title}
                  fill
